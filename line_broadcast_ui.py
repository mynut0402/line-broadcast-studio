...
def download_update_worker(latest_version, download_url):
    download_path = APP_DATA_DIR / f"LINE-Broadcast-Studio-{latest_version}.new.exe"
    try:
        with requests.get(
            download_url,
...
def install_downloaded_update(download_path, latest_version):
    ...
    script = (
        ...
        "        break\n"
        "    } catch {\n"
        "        if (($i % 20) -eq 0) { Log ('copy retry ' + $i + ': ' + $_.Exception.Message) }\n"
        "        Start-Sleep -Milliseconds 500\n"
        "    }\n"
        "}\n"
        "if (-not $updated -and (Test-Path -LiteralPath $source)) {\n"
        "    try {\n"
        ...
        "        Remove-Item -LiteralPath $backup -Force -ErrorAction SilentlyContinue\n"
        "        $updated = $true\n"
        "        Log 'move fallback succeeded'\n"
        "    } catch {\n"
        "        Log ('move fallback failed: ' + $_.Exception.Message)\n"
