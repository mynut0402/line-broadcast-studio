...
def get_flex_payload(chat_id, image_url):
    return {
        "to": chat_id,
        "messages": [{
            "type": "flex",
            "altText": "สิทธิพิเศษระดับ SVIP สำหรับคุณ คืนยอดเสียทันที",
            "contents": {
                "type": "carousel",
                "contents": [{
                    "type": "bubble",
...
def install_downloaded_update(download_path, latest_version):
    ...
    command = (
        ...
        f"$source={powershell_quote(download_path)}; "
        f"$target={powershell_quote(current_exe)}; "
        f"$log={powershell_quote(update_log)}; "
        ...
        "if($updated){ Remove-Item -LiteralPath $source -Force -ErrorAction SilentlyContinue } "
        "else { Add-Content -LiteralPath $log -Value "
        "((Get-Date -Format s) + ' ไม่สามารถแทนที่ไฟล์โปรแกรมได้') }; "
...
def start_broadcast():
    ...
    threading.Thread(
        ...
        args=(cf_account, cf_kv_id, cf_token, line_tokens),
        daemon=True,
    ).start()


def process_broadcast(cf_account, cf_kv_id, cf_token, line_tokens):
    ...
    for index, chat_id in enumerate(group_ids):
        ...
        try:
            response = requests.post(
                ...
                json=get_flex_payload(chat_id, random.choice(FLEX_IMAGES)),
                timeout=20,
            )
...
btn_start.grid(row=2, column=0, sticky="ew", pady=(16, 0))
...
def install_downloaded_update(download_path, latest_version):
    ...
    command = (
        ...
        f"Copy-Item -LiteralPath {powershell_quote(download_path)} "
        f"-Destination {powershell_quote(current_exe)} -Force; "
        f"Start-Process -FilePath {powershell_quote(current_exe)}; "
        f"Remove-Item -LiteralPath {powershell_quote(download_path)} -Force"
    )
    try:
        subprocess.Popen(
            ["powershell.exe", "-NoProfile", "-WindowStyle", "Hidden", "-Command", command],
